...
dados_tratados[,':='(bin_0 = factor(dados_originais$bin_0),
                     ...
                     ord_0 = factor(dados_originais$ord_0, ordered = TRUE),
                     ord_1 = factor(dados_originais$ord_1,
                                    levels = c("Novice","Contributor","Expert","Master","Grandmaster"),        
                                    ordered = TRUE),
                     ord_2 = factor(dados_originais$ord_2, 
                                    levels = c("Freezing","Cold","Warm","Hot","Boiling Hot","Lava Hot"),
                                    ordered = TRUE),
                     ord_3 = factor(dados_originais$ord_3, ordered = TRUE),
                     ord_4 = factor(dados_originais$ord_4, ordered = TRUE),
                     ord_5 = factor(dados_originais$ord_5, ordered = TRUE),
                     day = as.integer(dados_originais$day),
                     month = as.integer(dados_originais$month),
...
# Retira as variáveis day e month
dt <- dados_tratados[,c(1:18,21),with=F]
str(dt)
anyNA(dt)
...
# Criando o pré-processmaneto dos dados
options(warn = -1)
set.seed(215)
tratamento <- preProcess(dt,
                         method = c("center"))
tratamento
ls(tratamento)
tratamento$median

dt_tratado <- predict(tratamento, dt)
anyNA(dt_tratado)
