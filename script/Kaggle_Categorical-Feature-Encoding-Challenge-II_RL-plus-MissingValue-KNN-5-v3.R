...
# ----- Tratando os NA's com KNN para os dados de treino
arquivo <- 'dt-Treino_MissingValue_KNN-5.csv'
if (file.exists(paste('./',arquivo, sep = ''))) {
  ...
  (i <- Sys.time())
  system.time(dados_treino_1 <- knnImputation(dados_treino_1[,!names(dados_treino_1) %in% "target", with=F], 
                                              k = 5, 
                                              meth = 'median'))
  (f <- Sys.time())
  
  # OBS: O uso do KNN para o preenchimento dos NA's adiciona uma camada preditiva no tratatmento de dados.
  # O uso de KNN é comum para preenchimento de NA's pois consegue ser robusto e simples ao mesmo tempo.
...
# Reduzindo o total de classes das variáveis nom_5, nom_6 e nom_9
dados_treino_1[, ':='(nom_5 = ceiling(dados_treino_1$nom_5/10),
                      nom_6 = ceiling(dados_treino_1$nom_6/10),
                      nom_9 = ceiling(dados_treino_1$nom_9/10))]

head(dados_treino_1$nom_5)
length(unique(dados_treino_1$nom_5))
head(dados_treino_1$nom_6)
length(unique(dados_treino_1$nom_6))
head(dados_treino_1$nom_9)
length(unique(dados_treino_1$nom_9))

dados_treino_1[,':='(nom_5 = as.factor(dados_treino_1$nom_5),
                     nom_6 = as.factor(dados_treino_1$nom_6),
                     nom_9 = as.factor(dados_treino_1$nom_9))]
...
saveRDS(mc, file = 'matrizConfusao-KNN-5-V3.RDS')
...
# ----- Tratando os NA's com KNN para os dados de teste
arquivo <- 'dt-Teste_MissingValue_KNN-5.csv'
if (file.exists(paste('./',arquivo, sep = ''))) {
  ...
  (i <- Sys.time())
  system.time(dados_teste_1 <- knnImputation(dados_teste_1[,!names(dados_teste_1) %in% "target", with=F],
                                             k = 5,
                                             meth = 'median'))
  (f <- Sys.time())
...
# Reduzindo o total de classes das variáveis nom_5, nom_6 e nom_9
dados_teste_1[, ':='(nom_5 = ceiling(dados_teste_1$nom_5/10),
                     nom_6 = ceiling(dados_teste_1$nom_6/10),
                     nom_9 = ceiling(dados_teste_1$nom_9/10))]

head(dados_teste_1$nom_5)
length(unique(dados_teste_1$nom_5))
head(dados_teste_1$nom_6)
length(unique(dados_teste_1$nom_6))
head(dados_teste_1$nom_9)
length(unique(dados_teste_1$nom_9))

dados_teste_1[,':='(nom_5 = as.factor(dados_teste_1$nom_5),
                    nom_6 = as.factor(dados_teste_1$nom_6),
                    nom_9 = as.factor(dados_teste_1$nom_9))]
...
fwrite(previsao_final, file = './prevFinal-KNN-5-v3.csv')
