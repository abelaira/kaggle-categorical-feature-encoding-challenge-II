...
# ----- Tratando os NA's com a moda local para os dados de treino
arquivo <- 'dt-Treino_MissingValue_Moda-Local.csv'
if (file.exists(paste('./',arquivo, sep = ''))) {
  ...
  for (y in 1:5) {
    for (ind in 1:(length(which(vet_na[,y])))) {
      ...
      z <- table(dados_treino_1[(x-3):(x+3),y,with=F])
...
str(previsao)
confusionMatrix(data = previsao, reference = dt_teste$target)
...
# ----- Tratando os NA's com a moda local para os dados de teste
arquivo <- 'dt-Teste_MissingValue_Moda-Local.csv'
if (file.exists(paste('./',arquivo, sep = ''))) {
  ...
  for (y in 1:5) {
    for (ind in 1:(length(which(vet_na[,y])))) {
      x <- which(vet_na[,y])[ind]
      z <- table(dados_teste_1[(x-3):(x+3),y,with=F])
...
id <- 600000:999999
previsao_final[, ':='(id = as.integer(id),
                      target = ifelse(pred_rl_teste >= cutoff, 1, 0))]

str(previsao_final)
head(previsao_final, n = 10)
fwrite(previsao_final, file = './prevFinal-ModaLocal.csv')
